...
export default {
    data() {
        return {
            ...
            // 管理餐廳後端撈的所有資料
            allRestaurantt: [],
            //編輯設施的時候要有一個舊name讓資料庫可以找因為我的pk是名字
            editOldNameRest: "",
            // 管理餐廳搜尋欄綁的變數
            searchNameRest: "",
            searchPlaceRest: "",
    ...
    methods: {
        ...
        goAddRestaurant() {
            ...
            //設定照片
            const img = document.getElementById('addRestaurantImg')
            img.src = ""
        ...
        // 新增餐廳-選擇照片(未修)
        handleFileChangeRestaurant(event) {
            // 獲取文件資料
            const file = event.target.files[0];
        ...
        onSubmitRestaurant() {
            ...
            const url = "http://localhost:8080/api/restaurant/create";
            const data = {
                "restaurant": {
                    "name": this.nameRest,
        ...
        searchAllRestaurant() {
            const url = 'http://localhost:8080/api/restaurant/getAllRestaurantFromBack';
            // 要帶入的值
        ...
        searchRestCondition() {
            this.openFullScreen2();
            // 假如是否開放有帶到的話用這個三個條件的方法
            if (this.searchOpenRes !== "") {
                const url = 'http://localhost:8080/api/restaurant/searchThreeCondition';
                // 要帶入的值

                const queryParams = new URLSearchParams({
                    name: this.searchNameRes,
                    place: this.searchPlaceRes,
                    published: this.searchOpenRes
                });
            }
        },
        //人員管理--新增人員
        ...
        searchAdminUser() {
            ...
            const urlRest = 'http://localhost:8080/api/restaurant/searchTwoCondition';
            // 要帶入的值

            const queryParams2 = new URLSearchParams2({
                nameRest: this.searchNameRest,
                placeRest: this.searchPlaceRest,
            });

            // 將查詢字串附加到 URL
            const urlWithParams2 = `${url}?${queryParams2}`;

            fetch(urlWithParams2, {
                method: "GET",
                headers: new Headers({
                    "Accept": "application/json",
                    "Content-Type": "application/json",
        ...
        delRestaurant(index) {
            ...
            const url = 'http://localhost:8080/api/restaurant/deleteRestaurant';
            // 要帶入的值

            const queryParams = new URLSearchParams({
                nameRest: this.allRestaurant[index].name,
            });
        ...
        // 管理餐廳-更新餐廳
        updateRestaurant() {
            const url = 'http://localhost:8080/api/restaurant/updateRestaurant';
            const queryParams = new URLSearchParams({
            ...
            const data = {
                "restaurant": {
                    "nameRest": this.nameRest,
                    "descriptionRest": this.descRest,
                    "placeRest": this.regionRest,
                    "publishedRest": this.publishedRest,
                    "photoSRest": this.photoSRest,
                    "photoRest": null,
                    "startDateRest": this.date1Rest,
                    "endDateRest": this.date2Rest
                }
            };
            ...
            fetch(urlWithParams, {
                ...
                .then(data => {
                    console.log(data);
                    return this.searchRestCondition();
                });
...
<template>
    <div class="bigBg">
        ...
        <div class="rightBg">
            ...
            <!-- 管理餐廳 -->
            <div v-show="changePageNum == 4" class="manageRestaurant">
                <div class="manageTop">
                    <div class="search">
                        <h4>搜尋名稱</h4>
                        <input v-model="this.searchName"
                    ...
                    <div class="itemBlock" v-for="restaurant, index in this.allRestaurant">
                        ...
                        <div class="BtnPlace">
                            <button :key="index" @click="goEditRestaurant(index)" type="button"><i
                                    class="fa-solid fa-pen-to-square"></i></button>
                            <button :key="index" @click="sureDeleteRestaurant(index)" type="button"><i
                                    class="fa-solid fa-trash"></i></button>
                        </div>
                ...
                <div v-show="changePageNum == 5" class="manageManager">
                    ...
                    <div class="PeoplemanageBot">
                        <div class="itemBlock" v-for="adminuser, index in this.allAdminUser">
                            ...
                            <div class="BtnPlace">
                                <button :key="index" @click="" type="button"><i
                                        class="fa-solid fa-pen-to-square"></i></button>
            ...
            <!-- 新增餐廳選項 -->
            <div v-show="changePageNum == 5" class="addRestaurant">
                ...
                <el-form class="formPlace" :model="form" label-width="120px">
                    ...
                    <el-form-item label="選擇照片">
                        <div class="photoPlace">
                            <label for="file-upload" class="custom-file-upload">選擇照片</label>
                            <input id="file-upload" type="file" @change="handleFileChangeRestaurant">
                            <img id="addFacilityImg" src="" ref="preview"
                                style="display: none; max-width: 200px; max-height: 200px;">
                        </div>
                    </el-form-item>
            ...
            <!-- 編輯餐廳選項 -->
            <div v-show="changePageNum == 6" class="addRestaurant">
